...
--[[
    ...
    -- 如果辅助码只有两个键，且都匹配上，则返回true
    if #auxStr == 2 then
    -- 当第一个键和第二个键都匹配时，才返回 true
    ...
    for j=1,#auxStr do
        if not fullAux[j] then
            KeyMatched[j] = false -- 辅码不够长
        else
            KeyMatched[j] = (fullAux[j]:find(auxStr:sub(j, j)) ~= nil)
        end
    end
